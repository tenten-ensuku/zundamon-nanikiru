...
<main class="app"><section class="top"><h1>何切る 動画照合</h1><div class="controls"><select id="question"></select><select id="view"><option value="candidates">自動候補（上位12）</option><option value="all">全180件</option></select><input id="search" placeholder="動画番号・タイトルで絞り込み"><button id="export" class="export">対応表CSVを保存</button></div><div id="status" class="status"></div></section><section class="compare"><article class="question" id="questionCard"></article><section id="videos" class="videos"></section></section></main>
<script>
...
function renderVideos(){const needle=search.value.trim().toLowerCase();const qid=selected();const ranked=questionCandidates[String(qid)]||[];const rankByVideo=new Map(ranked.map((candidate,index)=>[candidate.videoId,{rank:index+1,score:candidate.score}]));const base=view.value==='all'?matches:ranked.map(candidate=>matches.find(match=>match.videoId===candidate.videoId)).filter(Boolean);const visible=base.filter(m=>!needle||String(m.playlistIndex).includes(needle)||m.title.toLowerCase().includes(needle));status.textContent=`問題${qid}を選択中。${view.value==='all'?'全件':'自動候補'} ${visible.length}件を表示 / 確定済み ${Object.keys(mapping).length}件`;videos.replaceChildren(...visible.map(m=>{const card=document.createElement('article');card.className='card'+(mapping[qid]?.videoId===m.videoId?' matched':'');const frame=m.frames?.[0]||'';const rank=rankByVideo.get(m.videoId);card.innerHTML=`<img src="${frame}" alt="動画${m.playlistIndex}の問題画面"><div class="body"><strong>${m.playlistIndex}. ${m.title}</strong><div class="meta">${rank?`候補 ${rank.rank}位 / 牌姿類似度 ${rank.score}`:'候補外'}</div><a href="${m.url}" target="_blank" rel="noreferrer">YouTubeを開く</a><button type="button">この問題に対応付け</button></div>`;card.querySelector('button').addEventListener('click',()=>assign(m));return card}))}
