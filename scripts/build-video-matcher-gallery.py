...
<main class="app"><section class="top"><h1>何切る 動画照合</h1><div class="controls"><select id="question"></select><input id="search" placeholder="動画番号・タイトルで絞り込み"><button id="export" class="export">対応表CSVを保存</button></div><div id="status" class="status"></div></section><section class="compare"><article class="question" id="questionCard"></article><section id="videos" class="videos"></section></section></main>
<script>
const questionImageBase='../../public/questions/';
const storeKey='zundamon-video-matcher-v1';
let questions=[],matches=[],mapping=JSON.parse(localStorage.getItem(storeKey)||'{}');
const qSelect=document.querySelector('#question'), search=document.querySelector('#search'), videos=document.querySelector('#videos'), status=document.querySelector('#status'), qCard=document.querySelector('#questionCard');
...
function renderVideos(){const needle=search.value.trim().toLowerCase();const qid=selected();const visible=matches.filter(m=>!needle||String(m.playlistIndex).includes(needle)||m.title.toLowerCase().includes(needle));status.textContent=`問題${qid}を選択中。動画${visible.length}件を表示 / 確定済み ${Object.keys(mapping).length}件`;videos.replaceChildren(...visible.map(m=>{const card=document.createElement('article');card.className='card'+(mapping[qid]?.videoId===m.videoId?' matched':'');const frame=m.frames?.[0]||'';card.innerHTML=`<img src="${frame}" alt="動画${m.playlistIndex}の問題画面"><div class="body"><strong>${m.playlistIndex}. ${m.title}</strong><div class="meta">自動候補: ${m.bestQuestionId??'—'} / ${m.confidence??'未判定'}</div><a href="${m.url}" target="_blank" rel="noreferrer">YouTubeを開く</a><button type="button">この問題に対応付け</button></div>`;card.querySelector('button').addEventListener('click',()=>assign(m));return card}))}
...
Promise.all([fetch('gallery-questions.json').then(r=>r.json()),fetch('matches.json').then(r=>r.json())]).then(([q,report])=>{questions=q;matches=report.matches;q.forEach(x=>qSelect.add(new Option(`問題 ${x.id}`,x.id)));qSelect.addEventListener('change',()=>{renderQuestion();renderVideos()});search.addEventListener('input',renderVideos);document.querySelector('#export').addEventListener('click',exportCsv);renderQuestion();renderVideos()});
