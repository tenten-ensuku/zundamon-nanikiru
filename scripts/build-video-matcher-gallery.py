...
function renderVideos(){const needle=search.value.trim().toLowerCase();const qid=selected();const ranked=questionCandidates[String(qid)]||[];const rankByVideo=new Map(ranked.map((candidate,index)=>[candidate.videoId,{rank:index+1,score:candidate.score,doraScore:candidate.doraScore}]));const base=view.value==='all'?matches:ranked.map(candidate=>matches.find(match=>match.videoId===candidate.videoId)).filter(Boolean);const visible=base.filter(m=>!needle||String(m.playlistIndex).includes(needle)||m.title.toLowerCase().includes(needle));status.textContent=`問題${qid}を選択中。${view.value==='all'?'全件':'ドラ画像が一致した候補'} ${visible.length}件を表示 / 確定済み ${Object.keys(mapping).length}件`;videos.replaceChildren(...visible.map(m=>{const card=document.createElement('article');card.className='card'+(mapping[qid]?.videoId===m.videoId?' matched':'');const frame=m.frames?.[0]||'';const rank=rankByVideo.get(m.videoId);card.innerHTML=`<img src="${frame}" alt="動画${m.playlistIndex}の問題画面"><div class="body"><strong>${m.playlistIndex}. ${m.title}</strong><div class="meta">${rank?`ドラ一致 / 牌姿候補 ${rank.rank}位 (${rank.score})`:'ドラ候補外'}</div><a href="${m.url}" target="_blank" rel="noreferrer">YouTubeを開く</a><button type="button">この問題に対応付け</button></div>`;card.querySelector('button').addEventListener('click',()=>assign(m));return card}))}
